...
uint8_t KeyboardBase::getKeyCode(uint8_t k, uint8_t* modifiersP)
{
	*modifiersP = 0;
	if (mode == ASCII_TO_HID) 
	{
...
size_t KeyboardBase::press(uint8_t k) {
	uint8_t modifiers;
...
size_t KeyboardBase::release(uint8_t k)
{
	uint8_t modifiers;
	k = getKeyCode(k, &modifiers);
...
void KeyboardBase::releaseAll(void)
{
...
void KeyboardBase::setKeyboardMode(KeyboardMode newmode)
{
	mode = newmode;
}

size_t KeyboardBase::write(uint8_t c)
{
	if (press(c)) {
		release(c);		// Keyup
		return 1;
...
void KeyboardBase::onLED(LedCallbackFcn fcn, void* cbData) {
	ledCB = fcn;
	ledCBdata = cbData;
}
